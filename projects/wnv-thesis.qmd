...
The pipeline and modeling harness are built, and I've stress-tested them on
...
simulation study takes real public covariate structure, simulates outcomes with
a known effect, and then checks whether each candidate model's 95% intervals
cover that effect at the stated rate, across scenarios that vary temporal
autocorrelation, overdispersion, and positivity. One clear result so far:
leaving out seasonal and month-to-month temporal structure badly under-covers
the uncertainty intervals, and the point estimates alone don't reveal the
problem. That's a finding about the modeling framework, not about WNV.

The analysis on actual surveillance data is gated on data-use and review
requirements, so no substantive results appear here. This page describes the
...
- Putting surveillance effort and detection into the model instead of ignoring
  them.
